...
class DQN:
    ...
    def choose_action(self, state):
        state = th.tensor(state, dtype=th.float32).to(device) # float32
        if np.random.uniform() <= self.eps:
            action = np.random.randint(0, self.action_dim)  # discrete action 0~n-1
        else:
            action_value = self.q_net(state)
            action = th.max(action_value, dim=-1)[1].cpu().numpy()  # tensor->numpy  float32->int
        return int(action)
    ...
    def learn(self):
        ...
        state = th.tensor(state, dtype=th.float32).to(device) # state, state
        actions = th.tensor(actions, dtype=th.long).to(device) # discrete
        ...
        q_eval = self.q_net(state).gather(-1, actions) # q evaluate
        q_next = self.target_net(next_state).detach() 
        q_target = rewards + self.args.gamma * (1 - dones) * th.max(q_next, dim=-1, keepdim=True)[0]  # Q_target = r + gamma * q_next
...
def main(args, logger=None):
    ...
    for i_episode in range(args.n_episodes):
        ...
        while not done:
            ...
            next_state, reward, terminated, truncated, info = env.step(action)  # gym==0.26
            done = terminated or truncated
            agent.store_transition(state, action, reward, next_state, done)
            episode_reward += reward
            state = next_state
            if agent.buffer.size >= args.buffer_size: # batch_size, buffer_size
                agent.learn()
        if args.use_wandb:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--use_wandb", default=False, type=bool)  # weight and bias sweep
