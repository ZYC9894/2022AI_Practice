...
class QNet(nn.Module):
    def __init__(self, input_size, hidden_size, output_size):
        ...
        self.fc2_V = nn.Linear(hidden_size, 1)
        self.fc2_A = nn.Linear(hidden_size, output_size)
...
class DQN:
    ...
    def choose_action(self, state):
        ...
        if np.random.uniform() <= self.eps:
            ...
            _, advantage_value = self.q_net(state)
            action = th.max(advantage_value, dim=-1)[1].cpu().numpy()
        return int(action)
    ...
    def learn(self):
        ...
        V_eval, A_eval = self.q_net(state)
        q_eval = (V_eval + A_eval - th.mean(A_eval, dim=-1, keepdim=True)).gather(-1, actions)
        V_eval_next, A_eval_next = self.target_net(next_state)
        q_next = V_eval_next.detach() + A_eval_next.detach() - th.mean(A_eval_next.detach(), dim=-1, keepdim=True)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--algorithm", default="Dueling_DQN", type=str)
