...
def main(args, logger=None):
    ...
    # init network
    shared_layer = SharedLayer(obs_dim, args.hidden).to(device)
    actor = Actor(input_dim=args.hidden, shared_layer=shared_layer, out_dim=action_dim).to(device)
    actor_optim = th.optim.Adam(actor.parameters(), lr=args.a_lr)
    critic = Critic(input_dim=args.hidden, shared_layer=shared_layer, out_dim=1).to(device)
